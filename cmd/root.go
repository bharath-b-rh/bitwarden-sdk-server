...
func init() {
	...
	flag.StringSliceVar(&rootArgs.server.TLSCurvePreferences, "tls-curve-preferences", nil,
		"ordered list of TLS key exchange curves for the server. "+
			"Full lists of available curves can be found at https://pkg.go.dev/crypto/tls#pkg-constants. "+
...
func runServeCmd(cmd *cobra.Command, _ []string) error {
	...
	if _, err := svr.TLSConfig(); err != nil {
		return err
	}
